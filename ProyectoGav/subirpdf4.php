                      ...
                      <div class="accordion-body">
                        ...
                        <script>
...

                            {
                              ...
                              document.getElementById('Financiamiento02').value = opciones[opcion][1];
                        ...
                        <form method="post"  class="row g-3">
                              ...
                              <label>Tipo de cotizacion:  </label>
                                ...
                                <div class = "col-md-4">
                                  <!--tipo de impuestos-->
                                  <a>Impuesto %</a>
                                  <br><input class="form-control text-end" type='text' id='Financiamiento02' disabled readonly/>
                                </div>
...
function AgregarCampos(){
    ...
    campo = `<tr id="idLi${nextinput}">


  
            <td><input  type="text" size="11" name="No[]"  disabled></td>
            <td><input  type="text" size="11" name="Acciones[]"></td>
            <td><input  type="text" size="12" name="Clave[]"></td>
            <td><input  type="text" size="12" name="Informacion[]" disabled></td>


            <!--Costo del producto-->
            <td><input  type="text" size="12" name="txt[]" id ="txt[]" step="any" oninput="multiplicar(event);"></td>
            <!--Cantidad del producto-->
            <td><input  type="text" size="11" name="num2[]" id ="num2[]"  step="any" oninput="multiplicar(event);"></td>

            <!--Descuento en %-->
            <td><input  type="text" size="12" maxlength ='2' name="Des[]"   step="any" oninput="multiplicar(event);"></td>
            <td><input  type="text" size="12" name="DesUni[]"   step="any"disabled "></td>
            <td> <input  type="text" size="12" name="Destotal[]"   step="any" disabled "></td>

            <!--Financiamiento-->
            <td><input  type="text" size="11" name="Fin[]" id='opciones[]'  step="any" oninput="multiplicar(event);"></td>
            <td> <input  type="text" size="12" name="FinUni[]" id ="FinUni[]"  step="any" disabled "></td>
            <td> <input  type="text" size="12" name="FinTotal[]" id ="FinTotal[]"  step="any" disabled "></td>

            <!--Calculo de IVA-->
            <td> <input  type="text" size="11" name="IVA[]" id ="IVA[]"  step="any" disabled "></td>

            <!--Precio unitario-->
            <td> <input  type="text" size="12" name="Preciouni[]" id ="Preciouni[]"  step="any" disabled "></td>

            <td><input  type="text" size="12" name="acumu[]" id ="cumu[]"  step="any" disabled></td>
            <!--subtotal oculto-->
            <td> <input hidden type="text" size="12" name="Subtotal[]" id ="PrecioFin[]"  step="any" disabled ></td>

            <!--Precio Final-->
            <td> <input  type="text" size="12" name="PrecioFin[]" id ="PrecioFin[]"  step="any" disabled ></td>
        </tr>`;
...
function multiplicar(e) {
    ...
    let textos = $(tr).find('[name^="txt"]').val().split('Precio:');
    // Obtener precio desde el último elemento de textos
    let precio = parseInt(textos[textos.length - 1]) || 0;
    // Obtener cantidad
    let cantidad = parseInt($(tr).find('[name^="num2"]').val()) || 0;
    let Des = parseInt($(tr).find('[name^="Des"]').val()) || 0;
    let Fin = parseInt($(tr).find('[name^="Fin"]').val()) || 0;
    

    //Descuento unitario
    $(tr).find('[name^="DesUni"]').val(precio * (Des/100));
    //Descuento total
    $(tr).find('[name^="Destotal"]').val( cantidad * (precio * (Des/100)));
    //Precio unitario
    $(tr).find('[name^="Preciouni"]').val((precio - (precio * (Des/100)))+((precio - (precio * (Des/100))) * (Fin/100)));

     //Financiamiento unitario
     $(tr).find('[name^="FinUni"]').val((precio - (precio * (Des/100))) * (Fin/100));
    //Financiamiento total
    $(tr).find('[name^="FinTotal"]').val( cantidad * ((precio - (precio * (Des/100))) * (Fin/100)));

    //IVA
    $(tr).find('[name^="IVA"]').val(0.16 * (((precio * cantidad)-( cantidad * (precio * (Des/100))))+( cantidad * ((precio - (precio * (Des/100))) * (Fin/100))) ));

    //Importe inicial
    $(tr).find('[name^="acumu"]').val(precio * cantidad);

    //Importe subtotal
    $(tr).find('[name^="Subtotal"]').val((((precio * cantidad)-( cantidad * (precio * (Des/100))))+( cantidad * ((precio - (precio * (Des/100))) * (Fin/100))) ));
    
     //Importe final
     $(tr).find('[name^="PrecioFin"]').val((((precio * cantidad)-( cantidad * (precio * (Des/100))))+( cantidad * ((precio - (precio * (Des/100))) * (Fin/100))) )
    + (0.16 * (((precio * cantidad)-( cantidad * (precio * (Des/100))))+( cantidad * ((precio - (precio * (Des/100))) * (Fin/100))) )));
    ...
    $('[name^="Destotal"]').each((index, item) => {
        // Incrementar total
        Destotal += parseFloat($(item).val());
    });
    $('[name^="FinTotal"]').each((index, item) => {
        // Incrementar total
        Fintotal += parseFloat($(item).val());
    });
    $('[name^="Subtotal"]').each((index, item) => {
        // Incrementar total
        Subtotal += parseFloat($(item).val());
    });
    $('[name^="IVA"]').each((index, item) => {
        // Incrementar total
        IVATotal += parseFloat($(item).val());
    });
    $('[name^="PrecioFin"]').each((index, item) => {
        // Incrementar total
        CompletoTotal += parseFloat($(item).val());
    });
    $('#total').val(total);
    $('#Destotal').val(Destotal);
    $('#Fintotal').val(Fintotal);
    $('#Subtotal').val(Subtotal);
    $('#IVATotal').val(IVATotal);
    $('#CompletoTotal').val(CompletoTotal);
}
</script>
<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.1.0/jquery.min.js"></script>
    <form id="form" name="form" method="post">
        <button type="button" class="btn btn-primary"  onclick="AgregarCampos();">Agregar servicio</button>
        <br><br>  
        <div class="table-responsive">
            <table class ="table table-bordered">
                <thead class="table-success table-striped">
                    ...
                    <tr>
                        <td>
                            Importe: <input type="text" id="total" value="0" disabled> <br>
                            Descuento:  <input type="text" id="Destotal" value="0" disabled><br>
                            Financiamiento:  <input type="text" id="Fintotal" value="0" disabled><br>
                            Subtotal:  <input type="text" id="Subtotal" value="0" disabled><br>
                            IVA:  <input type="text" id="IVATotal" value="0" disabled><br>
                            Total:  <input type="text" id="CompletoTotal" value="0" disabled><br>
                        </td>
                    </tr>
            </table>
        </div>
